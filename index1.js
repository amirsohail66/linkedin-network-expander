Linkedin = {
  ...
  sendInvites: function (data, config) {
    ...
    if (config.maxRequests == 0) {
      ...
      if (config.addNote && config.note) {
        ...
        console.debug(
          "DEBUG: clicking 'Send without a note' in popup, if present, in " +
            config.actionDelay +
            " ms"
        );
        setTimeout(() => this.clickSendWithoutNote(data, config), config.actionDelay);
      }
    }
  },
  ...
  clickSendWithoutNote: function (data, config) {
    var buttons = document.querySelectorAll("button");
    var sendWithoutNoteButton = Array.prototype.filter.call(buttons, function (el) {
      return el.textContent.trim() === "Send" || el.textContent.trim() === "Send without a note" || el.getAttribute('aria-label') === "Send now";
    });
    
    // Check for "Send without a note" or equivalent button
    if (sendWithoutNoteButton && sendWithoutNoteButton[0]) {
      console.debug("DEBUG: clicking 'Send without a note' button to close popup");
      sendWithoutNoteButton[0].click();
    } else {
      console.debug(
        "DEBUG: 'Send without a note' button not found, clicking close on the popup in " +
        config.actionDelay + " ms"
      );
    }
